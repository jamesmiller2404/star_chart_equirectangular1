...
##### Previous Unsupported Versions:

The following notes are for versions that had significant errors and are no longer available for download:

###### v3.7: Assign missing HR (Yale Bright Star Catalog) stars

98 HR (YBSC) IDs that were not found in HYG were investigated in more detail. Of these, 82 could be ascribed to HR IDs for multiple star components that are not in HIP, or to objects in YBSC that are not stellar (or otherwise unsuitable for HYG).
...
This was a major revision developed in 2012-2014, alongside the Endeavour (http://endeavour.astronexus.com) project. For a long time (2014-2023) it was _the_ HYG catalog. v3.0 was the first version to routinely include the full HIPPARCOS catalog. 

### Older databases: 

Version 1 was developed in the early 2000s to support some start chart programs I was working on at the time. It was significantly truncated compared to the full HIPPARCOS catalog (at most 64K entries).

Version 2 was developed in 2008, and added all the YBSC and Gliese stars, but only the brightest stars (brighter than about magnitude +9.0) in HIPPARCOS are included. Additionally, much less information was stored for each star.